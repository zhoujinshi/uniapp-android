...
function getWxEleId(index, arr) {//fixed by xxxxxx(临时使用随机数解决 slot 内引用组件导致 comid,eventid 冲突问题)
    if (!arr || !arr.length) {
        return "'" + makeRandom(3) + '-' + index + "'"
    }

    var str = arr.join("+'-'+")
    return "'" + makeRandom(3) + '-' + index + "-'+" + str
}
...
function transformDynamicClass(staticClass, clsBinding) {
    ...
    var cls = prettier
        .format(result.code, { semi: false, singleQuote: true })
        .slice(1)
        .slice(0, -1)
        .replace(/\n|\r/g, "")
...
function convertAst(node, options, util) {
    ...
    wxmlAst.attrsMap = attrs.format(wxmlAst.attrsMap)
		if (wxmlAst.attrsMap) { //fixed by xxxxxx
			if (wxmlAst.attrsMap.style) {
				wxmlAst.attrsMap.style = convertStaticStyle(wxmlAst.attrsMap.style)
			}
			//fixed by xxxxxx 微信小程序不支持在indicator-style中使用 rpx
			//         if (wxmlAst.attrsMap['indicator-style']) {
			//             wxmlAst.attrsMap['indicator-style'] = convertStaticStyle(wxmlAst.attrsMap['indicator-style'])
			//         }
		}
...
var REGEX_PX = /(:|\s|\(|\/)[+-]?\d+(\.\d+)?u?px/g
var REGEX_UPX = /(:|\s|\(|\/)[+-]?\d+(\.\d+)?upx/g

function convertStaticStyle(styleStr) {
    if (typeof styleStr === 'string') {
        var matches = styleStr.match(REGEX_UPX)
        if (matches && matches.length) {
            matches.forEach(function (match) {
                styleStr = styleStr.replace(match, match.substr(0, match.length - 3) + 'rpx')
            })
        }
        if (process.env.transformPx === 'true') { //需要转换 px
